...
cleanRmd <- function(RmdName = 'BycatchTripValidation_v2', RmdFolder = 'Markdown') {
  ...
  ii=i
	...
	tmp<-import(file.path("NewTrips",Fi[i]),sheet=1)
		...
		rm(tmp)
			...
			rmarkdown::render('Markdown/BycatchTripValidation_v2.Rmd', quiet=T)
			file.rename(from = file.path('Markdown','BycatchTripValidation_v2.pdf'), to = file.path('Reports',grp[1],nm))
			cleanRmd()
